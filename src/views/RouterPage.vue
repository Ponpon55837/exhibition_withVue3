<template>
  <div class='flex max-w-full justify-center' :class="darkMode ? 'bg-gray-400' : 'bg-gray-200' ">
    <div class="max-w-4xl shadow-lg hover:shadow-2xl border rounded-xl my-10 p-5">
      <h2 class="text-center mb-2 p-3 text-2xl font-bold rounded" :class="darkMode ? 'bg-gray-600':'bg-blue-200' ">{{ title }}</h2>
      <p class="text-xl mb-3">{{ des }}</p>
      ...
      <p v-if="sourceWebName" class="">購票系統：<a class="font-bold" :href="sourceWebPromote" :title="sourceWebName" target="_blank">{{ sourceWebName }}</a></p>
    </div>
  </div>
</template>
